...
from aleopantest.core.web_server import app
...
def test_get_admin():
    response = client.get("/aleopantest/api/admin")
...
def test_get_tools():
    response = client.get("/aleopantest/api/tools")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, dict)  # Grouped by category

def test_run_tool_missing_id():
    response = client.post("/aleopantest/api/run", json={"tool_id": "non_existent_tool", "target": "127.0.0.1"})
...
def test_run_tool_validation_error():
    ...
    response = client.post("/aleopantest/api/run", json={"target": "127.0.0.1"})

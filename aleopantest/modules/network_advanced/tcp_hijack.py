"""
TCP Hijack Detector - Aleopantest V4.0.0
...
class TCPHijack(BaseTool):
    """Detect TCP session hijacking"""

    def __init__(self):
        metadata = ToolMetadata(
            name="TCP Hijack Detector",
            category=ToolCategory.NETWORK_ADVANCED,
            version="4.0.0",
            author="Aleocrophic Team",
            description="Detect TCP session hijacking",
            usage="aleopantest run tcp-hijack --target <target>",
            requirements=[],
            tags=['network', 'tcp', 'hijack'],
            risk_level="HIGH",

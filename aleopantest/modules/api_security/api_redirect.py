"""
API Redirect Check - Aleopantest V4.0.0

Check for open redirect in APIs
"""
import time
import datetime
...
class APIRedirect(BaseTool):
    """Check for open redirect in APIs"""

    def __init__(self):
        metadata = ToolMetadata(
            name="API Redirect Check",
            category=ToolCategory.API_SECURITY,
            version="4.0.0",
            author="Aleocrophic Team",
            description="Check for open redirect in APIs",
            usage="aleopantest run api-redirect --target <target>",
            requirements=[],
            tags=['api', 'redirect', 'open'],
            risk_level="LOW",

...
from aleopantest.modules.wireless.deauth import DeauthTool
...
class TestDeauthTool(unittest.TestCase):
    ...
    @patch('aleopantest.modules.wireless.deauth.sendp')
    @patch('aleopantest.modules.wireless.deauth.HAS_SCAPY', True)
    def test_run_success(self, mock_sendp):
        # Mocking scapy components if needed, but since we mock sendp it should be fine
        with patch('aleopantest.modules.wireless.deauth.RadioTap'), \
             patch('aleopantest.modules.wireless.deauth.Dot11'), \
             patch('aleopantest.modules.wireless.deauth.Dot11Deauth'):
    ...
    @patch('aleopantest.modules.wireless.deauth.HAS_SCAPY', False)
